...
const TestimonialsSection = () => {
  ...
  const caseStudies = [{
    ...
    results: ["52% de reducción en exceso de inventario", "29% de aumento en la rotación de existencias", "Incremento del 18% en margen bruto"],
    bgColor: "bg-livs-blue"
  }, {
    industry: "Financiero",
    title: "Detección de Fraude Automatizada",
    results: ["73% de mejora en la detección de fraudes", "Reducción del 68% en falsos positivos", "Ahorro anual de €1.2M"],
  ...
  return <section id="casos" className="py-20 bg-livs-gray-light relative overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {caseStudies.map((study, index) => <div key={index} className={`${study.bgColor} rounded-xl text-white p-6 hover:shadow-xl transition-shadow duration-300`}>
              <div className="mb-3 font-medium text-sm opacity-80">{study.industry}</div>
              <h3 className="text-xl font-bold mb-4">{study.title}</h3>
              <ul className="space-y-2">
                {study.results.map((result, idx) => <li key={idx} className="flex items-start gap-2">
