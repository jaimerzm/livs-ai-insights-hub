  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a 
              key={link.title}
              href={link.href} 
              className="text-livs-blue-light font-medium text-sm hover:text-livs-purple transition-colors"
            >
              {link.title}
            </a>
          ))}
        ...
        {/* Mobile Menu Button */}
        <button 
          className="md:hidden text-livs-blue"
            ...
            <div className="container mx-auto px-4 py-4 flex flex-col">
              {navLinks.map((link) => (
                <a 
                  key={link.title}
                  href={link.href} 
                  className="text-livs-blue-light font-medium text-sm py-3 border-b border-gray-100 hover:text-livs-purple transition-colors"
