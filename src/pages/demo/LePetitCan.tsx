...
const LePetitCan = () => {
  ...
  useEffect(() => {
    ...
    const containerId = 'chatbot-widget-052872ec-48ce-476c-8ff8-d29f1dddb9b7';
    let container = document.getElementById(containerId);
    if (!container) {
      container = document.createElement('div');
      container.id = containerId;
      container.style.cssText = 'position:fixed;bottom:90px;right:24px;z-index:9999;';
    ...
    script.textContent = `
      (function() {
        ...
        function applyTheme(){var theme=THEMES[settings.interface_theme]||THEMES['classic-dark'];container._theme=theme;var ic=container.querySelector('.chatbot-inner-card');if(ic)ic.style.setProperty('background',theme.background,'important');var hdr=container.querySelector('.chatbot-header');if(hdr){setBg(hdr,theme.headerBg);hdr.style.setProperty('border-bottom-color',theme.inputBorder,'important');}var hTitle=container.querySelector('.chatbot-header h3');if(hTitle)hTitle.style.setProperty('color',theme.headerText,'important');var closeB=container.querySelector('.chatbot-close');if(closeB)closeB.style.setProperty('color',theme.headerText,'important');var msgArea=container.querySelector('.chatbot-messages');if(msgArea)msgArea.style.setProperty('background',theme.background,'important');var inArea=container.querySelector('.chatbot-input-area');if(inArea){inArea.style.setProperty('background',theme.background,'important');inArea.style.setProperty('border-top-color',theme.inputBorder,'important');}var inp=container.querySelector('.chatbot-input');if(inp){inp.style.setProperty('background',theme.inputBg,'important');inp.style.setProperty('border-color',theme.inputBorder,'important');inp.style.setProperty('color',theme.botText,'important');}var sendB=container.querySelector('.chatbot-send');if(sendB){setBg(sendB,theme.buttonBg);sendB.style.setProperty('color',theme.buttonText,'important');var svg=sendB.querySelector('svg');if(svg)svg.style.setProperty('color',theme.buttonText,'important');}var toggleB=container.querySelector('.chatbot-toggle');if(toggleB){toggleB.style.setProperty('background',theme.accent,'important');toggleB.style.setProperty('color',theme.buttonText,'important');var tSvg=toggleB.querySelector('svg');if(tSvg)tSvg.style.setProperty('color',theme.buttonText,'important');}var msgs=container.querySelectorAll('.chatbot-message');for(var i=0;i<msgs.length;i++)styleMsg(msgs[i]);}
        function styleMsg(msgDiv){var theme=container._theme;if(!theme)return;var bubble=msgDiv.querySelector('.chatbot-bubble');if(!bubble)return;var isUser=msgDiv.classList.contains('user');setBg(bubble,isUser?theme.userBubble:theme.botBubble);bubble.style.setProperty('color',isUser?theme.userText:theme.botText,'important');var kids=bubble.querySelectorAll(isUser?'*':'*:not(.chatbot-action-btn):not(.chatbot-action-btn *)');for(var j=0;j<kids.length;j++)kids[j].style.setProperty('color',isUser?theme.userText:theme.botText,'important');var btns=bubble.querySelectorAll('.chatbot-action-btn');for(var k=0;k<btns.length;k++){setBg(btns[k],theme.buttonBg.indexOf('gradient')!==-1?theme.buttonBg:theme.accent);btns[k].style.setProperty('color',theme.buttonText,'important');var bk=btns[k].querySelectorAll('*');for(var l=0;l<bk.length;l++)bk[l].style.setProperty('color',theme.buttonText,'important');}}
