...
const LePetitCan = () => {
  ...
  useEffect(() => {
    ...
    // The exact user-provided BotSupreme script
    const scriptContent = `
(function() {
  ...
  const CHATBOT_ID = '1f06407e-30de-4239-8095-f21b9b37c2f0';
  var THEMES = {
  ...
  function applyTheme() {
    ...
    if (toggleB) {
      toggleB.style.setProperty('background', theme.accent, 'important');
      toggleB.style.setProperty('color', theme.buttonText, 'important');
      var tSvg = toggleB.querySelector('svg');
      if (tSvg) tSvg.style.setProperty('color', theme.buttonText, 'important');
    }
  ...
  function styleMsg(msgDiv) {
    ...
    var kids = bubble.querySelectorAll(isUser ? '*' : '*:not(.chatbot-action-btn):not(.chatbot-action-btn *)');
    for (var j = 0; j < kids.length; j++) kids[j].style.setProperty('color', isUser ? theme.userText : theme.botText, 'important');
    var btns = bubble.querySelectorAll('.chatbot-action-btn');
    for (var k = 0; k < btns.length; k++) { setBg(btns[k], theme.buttonBg.indexOf('gradient') !== -1 ? theme.buttonBg : theme.accent); btns[k].style.setProperty('color', theme.buttonText, 'important'); var bk = btns[k].querySelectorAll('*'); for (var l = 0; l < bk.length; l++) bk[l].style.setProperty('color', theme.buttonText, 'important'); }
  }
  function escapeHtml(t) { var d = document.createElement('div'); d.textContent = t; return d.innerHTML; }
  function parseActionButtons(text) {
    var buttons = [];
    var clean = text;
    var s, e, content, parts;
    while ((s = clean.indexOf('[BTN:')) !== -1) {
      e = clean.indexOf(']', s);
      if (e === -1) break;
      content = clean.substring(s + 5, e);
      parts = content.split('|');
      if (parts.length === 3) {
        buttons.push({ label: parts[0].trim(), url: parts[1].trim(), type: parts[2].trim() });
      }
  ...
  function createButtonsHtml(buttons) {
    ...
    var theme = container._theme || THEMES['classic-dark'];
    var html = '<div class="chatbot-action-buttons">';
    for (var i = 0; i < buttons.length; i++) {
      var b = buttons[i]; var icon = b.type==='phone'?'\\u{1F4DE}':b.type==='email'?'\\u{1F4E7}':'\\u{1F517}';
      var href = escapeHtml(b.url); var tgt = b.type==='link'?' target="_blank" rel="noopener noreferrer"':'';
      html += '<a href="'+href+'"'+tgt+' class="chatbot-action-btn" style="background:'+theme.accent+';color:'+theme.buttonText+'"><span class="btn-label">'+icon+' '+escapeHtml(b.label)+'</span></a>';
    }
    return html + '</div>';
  }
  ...
  async function init() {
    ...
    addMessage('\\u00a1Hola! Soy tu asistente virtual. \\u00bfEn qu\\u00e9 puedo ayudarte hoy?', 'assistant', msgs);
    ...
    var isLoading = false;
    var history = [{ role: 'assistant', content: '\\u00a1Hola! Soy tu asistente virtual. \\u00bfEn qu\\u00e9 puedo ayudarte hoy?' }];
    ...
    function addMessage(text, role, target) {
      ...
      var avatarHtml = '';
      if (role === 'assistant' && settings.show_avatar && settings.bot_avatar_url) {
        avatarHtml = '<img src="'+settings.bot_avatar_url+'" style="width:32px;height:32px;border-radius:50%;object-fit:cover;flex-shrink:0" />';
      }
      var bubble = document.createElement('div');
      bubble.className = 'chatbot-bubble';
      if (role === 'user') { bubble.textContent = text; }
      else { var p = parseActionButtons(text); bubble.innerHTML = formatMarkdown(p.text) + createButtonsHtml(p.buttons); }
    ...
    function updateMessage(div, text) {
      ...
      if (bubble) { var p = parseActionButtons(text); bubble.innerHTML = formatMarkdown(p.text) + createButtonsHtml(p.buttons); styleMsg(div); msgs.scrollTop = msgs.scrollHeight; }
    }
    async function sendMessage() {
      ...
      try {
        ...
        var buf = ''; var done = false; var content = '';
        ...
        loadDiv.remove();
        var errMsg = 'Lo siento, no pude conectarme. Intenta de nuevo.';
